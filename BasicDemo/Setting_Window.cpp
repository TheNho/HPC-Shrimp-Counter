...
Setting_Window::Setting_Window(CWnd* pParent )
	...
	, setting_anpha(anpha)
	, setting_beta(beta)
	, setting_bsg_background_ratio(background_ratio)
	, setting_show_binary_image(show_binary_image)
{
...
void Setting_Window::DoDataExchange(CDataExchange* pDX) {
	...
	DDX_Check(pDX, IDC_CHECK_SHOW_BINARY_IMAGE, setting_show_binary_image);
}
...
bool Setting_Window::CheckParameters() { // Check parameters in current window
	...
	if (CheckFloat(get_anpha) == true) {
		if (_ttof(get_anpha) <= 0) {
			AfxMessageBox(L"anpha must be Positive!");
			return false;
		}
	}
	else {
		AfxMessageBox(L"anpha must be Float!");
		return false;
	}
...
bool Setting_Window::load_data_to_train_SVM(CString direction) { // Tested -> OK
	...
	svm->setTermCriteria(TermCriteria(TermCriteria::EPS + cv::TermCriteria::MAX_ITER,
									  10000,	// number iteration to compute
