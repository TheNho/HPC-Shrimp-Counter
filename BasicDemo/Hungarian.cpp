...
{
	unsigned int nnRows = DistMatrix.size();
	unsigned int nnCols = DistMatrix[0].size();

	double *distMatrixIn = new double[nnRows * nnCols];
	int *assignment = new int[nnRows];
	double cost = 0.0;
	...
	for (unsigned int i = 0; i < nnRows; i++)
		for (unsigned int j = 0; j < nnCols; j++)
			distMatrixIn[i + nnRows * j] = DistMatrix[i][j];
	
	// call solving function
	assignmentoptimal(assignment, &cost, distMatrixIn, nnRows, nnCols);

	Assignment.clear();
	for (unsigned int r = 0; r < nnRows; r++)
